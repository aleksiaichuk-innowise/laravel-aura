...
<head>
    ...
    <style>
        ...
        body { font-family: 'Outfit', sans-serif; background-color: #0c0e14; color: #e2e8f0; }
        .glass { background: rgba(30, 41, 59, 0.5); backdrop-filter: blur(10px); border: 1px solid rgba(255, 255, 255, 0.05); border-radius: 1rem; }
        .card-stat { transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); }
        .card-stat:hover { transform: translateY(-5px); border-color: #38bdf8; box-shadow: 0 10px 25px -5px rgba(56, 189, 248, 0.1); }
...
<body class="min-h-screen p-6 md:p-12">
    <div class="max-w-7xl mx-auto">
        <header class="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-12">
            <div>
                <h1 class="text-5xl font-bold bg-gradient-to-r from-sky-400 to-indigo-500 bg-clip-text text-transparent">Aura</h1>
                <p class="text-slate-500 mt-2 text-lg tracking-wide uppercase font-semibold">Self-Contained Performance Monitoring</p>
            </div>
        ...
        <main>
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
                <div class="glass p-6 card-stat">
                    ...
                    <div class="text-3xl font-bold mt-2 text-white glow-text">{{ number_format($memory->first()?->value ?? 0, 2) }} <span class="text-lg text-slate-500">MB</span></div>
                </div>
                <div class="glass p-6 card-stat">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left text-sm">
                            <tbody class="divide-y divide-white/5">
                                ...
                                @php $tags = is_string($http->tags) ? json_decode($http->tags, true) : $http->tags; @endphp
                                <tr class="hover:bg-white/5 transition-colors">
                                    <td class="px-6 py-4">
